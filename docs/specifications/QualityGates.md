...
- 静的検査は10分、Unityは30分、集約jobは5分でtimeoutする。
- Unityの`Library`をUnityバージョンとプロジェクトファイルのhashでキャッシュする。
- Unityの結果とログは成功・失敗を問わず14日間Artifactとして保存する。
...
`scripts/tests/quality-gate-tests.sh`で正常系、異常系、500／501行、200／201行などの境界値を検証する。`scripts/tests/git-hook-tests.sh`で初回導入、再実行、既存設定保護を検証する。
