...
4. 数字キーボードで宣言でき、コヨーテ、ラウンド継続、脱落、最終勝敗まで1ゲーム完走できる。
5. 再戦とTitle復帰が動作する。
6. ホーム移動でBGMが停止し、復帰後に設定どおり再開する。
7. 終了・再起動でTitleへ戻り、BGM設定だけが保持される。
8. 機内モードでも主要ゲームフローを完走できる。
9. ノッチ、角丸、ナビゲーション領域へ主要操作が隠れない。
